        ...
        <div class="card">
          <div class="card-header">
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                ...
                <a href="#" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" data-toggle="modal" data-target="#newsection"><i class="fas fa-file"></i>&nbspNew Section</a>
            </div>
          </div>
          <!-- /.card-header -->
          <div class="card-body">
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
                  @foreach ($sections as $section)
                      <tr>
                          <td>{{ $section->shelve }}</td>
                          <td>{{ $section->section }}</td>
                          <td>{{ $section->description }}</td>
                          <td>{{ $section->accessibility }}</td>
                          <td>{{ $section->attributes }}</td>
                          <td>{{ $section->contents }}</td>
                          <td>{{ $section->capacity }}</td>
                          <td>{{ $section->used_capacity }}</td>
                          <td>0%</td>
                          <td><a href="{{ $section->id }}" class="btn btn-danger"><i class= "fas fa-trash"></i></a></td>
                      </tr>
                  @endforeach
              </tbody>
            </table>
...
@section('modals')
<div class="modal fade" id="newsection">
          ...
          <div class="row">
              <div class="col-md-6">
                  <div class="form-group">
                      ...
                      <select name="shelve" class="form-control" required>
